...
public sealed class ScopeCatalogTests(PostgresFixture postgres)
{
    ...
    public async Task Fresh_catalog_grants_only_system_permissions_to_administrator()
    {
        ...
        Assert.Equal(4, await db.Database.SqlQueryRaw<int>("SELECT count(*)::int AS \"Value\" FROM permissions WHERE domain='scoped-business'").SingleAsync());
        Assert.Equal(8, await db.Database.SqlQueryRaw<int>("SELECT count(*)::int AS \"Value\" FROM permissions WHERE domain='system'").SingleAsync());
        await IdentityCatalog.SeedAsync(db, d.Clock.GetUtcNow(), default);
        await db.SaveChangesAsync();
