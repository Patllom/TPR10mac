    ...
    Identity.Authorization.PermissionContext? permission = null) : IAuditEventWriter
{
    public Task WriteAsync(SecurityAuditRequest request, CancellationToken ct)
    {
        ...
        string[] allowed = ["outcome", "scope", "target-type", "changed-fields", "acting-roles", "actor-type", "reason", "evidence-reference", "source", "capability", "scope-validation", "assignment-id", "row-count", "destination-type", "filter-from", "filter-to"];
