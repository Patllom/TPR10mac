...
export default async function OrganizationPage() {
  let content, session;
  try {session=await requirePortalSession('/portal/admin/organization');content=session.permissions.includes('organization:manage')?<OrganizationForm/>:<p role="alert">ไม่มีสิทธิ์จัดการโครงสร้างองค์กร</p>;}
  catch(error) {if(error instanceof SessionUnavailableError)content=<p role="alert">{error.message}</p>;else throw error;}
  return <ScopeFrame title="จัดการโครงสร้างองค์กร" session={session}>{content}</ScopeFrame>;
}
