...
export default async function AssignmentsPage() {
  let content;
  try {const session=await requirePortalSession('/portal/admin/assignments');content=session.permissions.includes('scope-assignments:manage')?<AssignmentForm actorId={session.userId}/>:<p role="alert">ไม่มีสิทธิ์จัดการการมอบหมาย</p>;}
  catch(error) {if(error instanceof SessionUnavailableError)content=<p role="alert">{error.message}</p>;else throw error;}
  return <ScopeFrame title="จัดการการมอบหมายสิทธิ์">{content}</ScopeFrame>;
}
